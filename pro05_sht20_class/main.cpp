#include "widget.h"
#include "ui_widget.h"

#include <QApplication>
#include <QObject>
#include <wiringPi.h>

widget *pW;
const int swPin[] = {4,17,27,22};
const int pirPin = 7;

void isrFunc1(void);
void isrFunc2(void);
void isrFunc3(void);
void isrFunc4(void);
void isrFuncPir(void);
void isrFuncAll(void);
void (*fp[])(void) = {isrFunc1,isrFunc2,isrFunc3,isrFunc4};
// thread
void* sht20Thread(void *arg);   // sht20 thread
void* pirThread(void *arg);     // pir thread

int main(int argc, char *argv[])
{
    QApplication a(argc, argv);
    widget w;
    pW = &w;
    w.show();

    for(int i=0; i<4; i++)
    {
       //  w.SW[i] = new GPIO_SW(swPin[i], INT_EDGE_RISING, fp[i]);
        w.SW[i] = new GPIO_SW(swPin[i], INT_EDGE_RISING, isrFuncAll);
    }

    w.PIR = new GPIO_SW(pirPin);

    w.SHT20 = new I2C_SHT20(SHT20_I2C_ADDR);

    // thread
    pthread_t pir_t_id;
    if(pthread_create(&pir_t_id,NULL, pirThread, NULL) != 0)
        qDebug("스레스 생성이 안됐습니다.\n");

    pthread_t sht20_t_id;
    if(pthread_create(&sht20_t_id,NULL, sht20Thread, NULL) != 0)
        qDebug("스레스 생성이 안됐습니다.\n");

    return a.exec();
}

void isrFunc1(void)
{
    if(pW->ui->label_SW_PIC1->isEnabled())
        pW->ui->label_SW_PIC1->setEnabled(false);
    else
        pW->ui->label_SW_PIC1->setEnabled(true);
}

void isrFunc2(void)
{
    if(pW->ui->label_SW_PIC2->isEnabled())
        pW->ui->label_SW_PIC2->setEnabled(false);
    else
        pW->ui->label_SW_PIC2->setEnabled(true);
}

void isrFunc3(void)
{
    if(pW->ui->label_SW_PIC3->isEnabled())
        pW->ui->label_SW_PIC3->setEnabled(false);
    else
        pW->ui->label_SW_PIC3->setEnabled(true);

}

void isrFunc4(void)
{
    if(pW->ui->label_SW_PIC4->isEnabled())
        pW->ui->label_SW_PIC4->setEnabled(false);
    else
        pW->ui->label_SW_PIC4->setEnabled(true);
}

void isrFunc5(void)
{
    if(pW->ui->label_PIR_PIC->isEnabled())
        pW->ui->label_PIR_PIC->setEnabled(false);
    else
        pW->ui->label_PIR_PIC->setEnabled(true);
}

void isrFuncAll(void)
{
    QLabel *SW_PIC[4] = {pW->ui->label_SW_PIC1,pW->ui->label_SW_PIC2,
                         pW->ui->label_SW_PIC3,pW->ui->label_SW_PIC4};

    for(int i=0; i<4; i++){
        if(pW->SW[i]->swRead()){
            if(SW_PIC[i]->isEnabled())
                SW_PIC[i]->setEnabled(false);
            else
                SW_PIC[i]->setEnabled(true);
        }
    }
}

void* sht20Thread(void *arg)
{
    while(1)
    {
        pW->ui->lcdNumber_Temp->setProperty("value", QVariant(pW->SHT20->measureTemp()));
        pW->ui->lcdNumber_Humi->setProperty("value", QVariant(pW->SHT20->measureHumi()));
        delay(1000);
    }
}

void* pirThread(void *arg)
{
    while(1)
    {
        if(pW->PIR->swRead()){
            pW->ui->label_PIR_PIC->setEnabled(true);
            delay(1000);
        }
        else
        {
            pW->ui->label_PIR_PIC->setEnabled(false);
        }
    }
}
